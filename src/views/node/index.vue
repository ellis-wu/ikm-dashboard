<template>
  <div class="node-container">
    ...
    <Row :gutter="16" type="flex" align="middle">
      <Col :xs="24" :sm="12":md="12" :lg="8" v-for="(item, index) in agentsDatas" :key="index" style="padding-top: 15px;">
        <div class="node-card">
          <div>
          ...
          <div v-if="item.metadata.labels['cluster-name'].length === 0" class="node-card__location">
            <Icon class="node-card__icon" type="android-alert" :size="14" color="#ed5e69"></Icon>
          ...
          <div style="margin-top: 20px; padding: 10px 0; border: 1px solid #e5e5e5">
            <Row :gutter="8">
              <Col span="8"
                ...
                <div style="text-align: center; font-size: 20px; font-weight: 600;">{{ agentInfo }}</div>
                <div style="text-align: center;">{{ agentKey }}</div>
              </Col>
            </Row>
          </div>

          <div style="margin-top: 20px; padding-left: 1px;">
            <span>{{ getLocalDateString(item.metadata.creationTimestamp) }}</span>
            <a style="float: right;" v-on:click="clickAgent(item)">more...</a>
          </div>
        </div>
      </Col>
    </Row>
...
import { fetchAgents } from '@/api/kubernetesCRD'
...
export default {
  ...
  methods: {
    ...
    getLocalDateString (date) {
      return new Date(date).toLocaleString(Cookies.get('language'))
    },
    getAgents () {
      fetchAgents().then(result => {
        this.agentsDatas = result.items.sort(function (a, b) {
          return a.status.hostInfo.hostname > b.status.hostInfo.hostname ? 1 : -1
        })
      })
    },
    getAgentsInfo (info) {
      return {
        cpu: info.status.hostInfo.cpu,
