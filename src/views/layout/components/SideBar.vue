    ...
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      <router-link v-if="item.children.length===1 && !item.children[0].children || item.onepage" :to="item.path+'/'+item.children[0].path" :key="item.children[0].name">
        <MenuItem :name="item.path+'/'+item.children[0].path">
          ...
          <span v-if="item.children[0].meta&&item.children[0].meta.title">{{translateKey(item.children[0].meta.title)}}</span>
        </MenuItem>
      </router-link>
      ...
      <Submenu v-else :name="item.name||item.path" :key="item.name">
        <template slot="title">
          ...
          <span v-if="item.meta&&item.meta.title">{{translateKey(item.meta.title)}}</span>
        </template>
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          <router-link :to="item.path+'/'+child.path" :key="child.name">
            <MenuItem :name="item.path+'/'+child.path">
              ...
              <span v-if="child.meta&&child.meta.title">{{translateKey(child.meta.title)}}</span>
            </MenuItem>
          </router-link>
        </template>
      </Submenu>
